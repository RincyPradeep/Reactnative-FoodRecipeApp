...
const Notification = () => {
    ...
    const notifications =[
        ...
        {
            id:7,
            title:"simply random text. It has roots in a piece of classical Latin literature from 45 BC, making it over 2000 years old.",
            read:true
        },
        {
            id:8,
            title:"available, but the majority have suffered alteration in some form, by injected humour.",
            read:true
        }
    ]
    ...
    <SafeAreaView style={styles.notificationContainer}>
      <FlatList
        ...
        renderItem = {({item})=>{
          return(
            <TouchableOpacity>
                <View style={[styles.notificationRow,{backgroundColor:!item.read && "#fff"}]}><Text style={styles.notificationText}>{item.title}</Text></View>
            </TouchableOpacity>
          )
        }}
      />
